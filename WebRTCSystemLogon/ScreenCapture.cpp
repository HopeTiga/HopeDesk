    ...
    namespace rtc {
        ...
        bool ScreenCapture::initializeProcessor()
        {
            ...
            if ((flags & D3D11_VIDEO_PROCESSOR_FORMAT_SUPPORT_OUTPUT) == 0) {
                return false; // 该显卡不支持 VP 输出 NV12
            }
            ...
            // 3. 【关键】初始化 CPU 回读用的 Staging 池
            texDesc.Usage = D3D11_USAGE_STAGING;
            texDesc.BindFlags = 0; // Staging 纹理不需要 BindFlags
            texDesc.CPUAccessFlags = D3D11_CPU_ACCESS_READ;
            texDesc.MiscFlags = 0; // 清除之前的 Flags

            for (int i = 0; i < YUV_BUFFERS; i++) {
                // 为每个池槽位创建一个 Staging Texture
        ...
        bool ScreenCapture::processFramePro(ID3D11Texture2D* texture)
        {
            ...
            // 这里的 proOutputTex 是 Default Usage (GPU端)
            ...
            if (targetFrame->isBusy.load()) {
                ...
                if (targetFrame->isBusy.load()) {
                    ...
                    if (!foundEmergency && emergencyNv12Buffers.size() != YUV_BUFFERS * 3) {
                        ...
                        stagingDesc.Format = DXGI_FORMAT_NV12; // 必须是 NV12
                        stagingDesc.SampleDesc.Count = 1;
                        stagingDesc.Usage = D3D11_USAGE_STAGING;           // CPU 读取专用
                        stagingDesc.BindFlags = 0;                         // Staging 纹理不需要绑定
                        stagingDesc.CPUAccessFlags = D3D11_CPU_ACCESS_READ; // 允许 CPU 读
