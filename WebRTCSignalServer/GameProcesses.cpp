    ...
    namespace entity {
        ...
        void GameProcesses::fromRow(const boost::mysql::row_view& row) {
            ...
            try {
                ...
                // 5: is_idle (tinyint(1), Ä¬ÈÏ true)
                is_idle = row.at(idx).is_null() ? true : (row.at(idx++).as_int64() != 0);

                // 6-7: startup_parameters, health_status
                ...
                // 11-12: created_at / updated_at (Ä¬ÈÏ CURRENT_TIMESTAMP)
