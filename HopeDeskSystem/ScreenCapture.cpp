...
namespace hope {
	namespace rtc {
		...
		bool ScreenCapture::initializeGPUConverter() {
			...
			if (config.levels == CaptureLevels::GPU) {
				...
				void main(uint3 id : SV_DispatchThreadID) {
					...
					for (int j = 0; j < 2; j++) {
						...
						[unroll]
						for (int k = 0; k < 2; k++) {
							uint y0 = GetY(c[k*4+0][j]);
							uint y1 = GetY(c[k*4+1][j]);
							uint y2 = GetY(c[k*4+2][j]);
							uint y3 = GetY(c[k*4+3][j]);
							outputBuffer[(row + blockX + k*4) >> 2] = (y3 << 24) | (y2 << 16) | (y1 << 8) | y0;
						}
					}
			...
			else if (config.levels == CaptureLevels::PRO) {
            ...
            void main(uint3 id : SV_DispatchThreadID) {
                ...
                float4 c00 = inputTexture.Load(int3(blockX, blockY, 0));
                float4 c10 = (blockX + 1 < width) ? inputTexture.Load(int3(blockX + 1, blockY, 0)) : c00;
                float4 c01 = (blockY + 1 < height) ? inputTexture.Load(int3(blockX, blockY + 1, 0)) : c00;
                float4 c11 = (blockX + 1 < width && blockY + 1 < height) ? inputTexture.Load(int3(blockX + 1, blockY + 1, 0)) : c00;
                
                uint y00 = GetY(c00), y10 = GetY(c10), y01 = GetY(c01), y11 = GetY(c11);
                
                uint yRow0 = blockY * yPitch;
                uint yRow1 = (blockY + 1) * yPitch;
                
                outputBuffer.Store(yRow0 + blockX + 0, y00);
                outputBuffer.Store(yRow0 + blockX + 1, y10);
                outputBuffer.Store(yRow1 + blockX + 0, y01);
                outputBuffer.Store(yRow1 + blockX + 1, y11);
                ...
                uint uvRow = blockY >> 1;
                uint uvOffset = ySize + uvRow * uvPitch + blockX;
                
                outputBuffer.Store(uvOffset + 0, u);
                outputBuffer.Store(uvOffset + 1, v);
            }
        )";
			}
			...
			UINT uvPitch = (config.width / 2 + 3) & ~3;
			UINT ySize = yPitch * config.height;
			UINT uSize = 0, vSize = 0, totalSize = 0;

			if (config.levels == CaptureLevels::GPU) {
				uSize = uvPitch * (config.height / 2);
				vSize = uvPitch * (config.height / 2);
				totalSize = ySize + uSize + vSize;
			}
			else {
				UINT uvSize = yPitch * (config.height / 2);  // NV12 UV 和 Y 行宽一样
				totalSize = ySize + uvSize;
			}
			...
			if (config.levels == CaptureLevels::GPU) {
				struct Constants { UINT width; UINT height; UINT yPitch; UINT uvPitch; UINT ySize; UINT uSize; };
				Constants consts = { (UINT)config.width, (UINT)config.height, yPitch, uvPitch, ySize, uSize };
				...
				struct Constants { UINT width; UINT height; UINT yPitch; UINT uvPitch; UINT ySize; };
				Constants consts = { (UINT)config.width, (UINT)config.height, yPitch, yPitch, ySize };
				D3D11_BUFFER_DESC cbDesc = {};
				cbDesc.ByteWidth = (sizeof(Constants) + 15) / 16 * 16;
				cbDesc.Usage = D3D11_USAGE_IMMUTABLE;
				cbDesc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
