...
namespace hope {
    namespace rtc {
        ...
        void VirtualDisplayCapture::captureThreadFunc()
        {
            ...
            const UINT32 idleReopenTicks = 30;  // 连续静止 30 圈（~3s）→ 主动重开探测
            UINT32 idleTicks = 0;
            ...
            while (capturing.load()) {
                ...
                if (wr == WAIT_OBJECT_0) {
                    ...
                    // 尽快触发下面的主动重开，避免依赖一个失效的事件句柄空转。
                    ++idleTicks;
                }

                // ---- 主动重开探测：连续静止超过阈值。休眠/唤醒时驱动可能整体
                // 重建 SwapChainProcessor+exporter（新事件/新映射/新纹理）。旧
                // pMeta 映射冻结，快速路径读不到新 generation。这里主动重开
                // 通道拿到新映射，从新映射读 generation 判断是否真的重建过。
                if (idleTicks >= idleReopenTicks) {
                    idleTicks = 0;
                    const UINT16 oldGen = lastChannelGen;
                    if (reopenFrameChannel()) {
                        ZakoFrameMetadata meta{};
                        if (readStableMetadata(meta)) lastChannelGen = static_cast<UINT16>(meta.MetadataSequence >> 16);
                        if (lastChannelGen != oldGen) {
                            // 驱动确实重建了通道：采纳新句柄，通知下游清缓存。
                            if (channelSync) channelSync->generation.fetch_add(1);
                            LOG_INFO("VirtualDisplayCapture channel rebuilt by producer (gen %u -> %u); reopened",
                                     oldGen, lastChannelGen);
                        }
                    }
                    else {
                        LOG_WARN("VirtualDisplayCapture channel reopen probe failed, will retry");
                        std::this_thread::sleep_for(std::chrono::milliseconds(250));
                    }
                }
            }
