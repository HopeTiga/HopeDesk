...
#include "chromiumShims/BaseMisc.h"
#include "chromiumShims/MediaDecoderBuffer.h"
#include "chromiumShims/MediaVideoTypes.h"
#include "chromiumShims/MediaAcceleratedVideoDecoder.h"
...
using AV1ReferenceFrameVector =
    std::array<webrtc::scoped_refptr<AV1Picture>, libgav1::kNumReferenceFrameTypes>;
...
class MEDIA_GPU_EXPORT AV1Decoder : public AcceleratedVideoDecoder {
 public:
  class MEDIA_GPU_EXPORT AV1Accelerator {
   public:
    ...
    virtual webrtc::scoped_refptr<AV1Picture> createAV1Picture(bool apply_grain) = 0;
    ...
    virtual webrtc::scoped_refptr<AV1Picture> createAV1PictureSecure(
        bool apply_grain,
        uint64_t secure_handle);

    // Submits |pic| to the driver for accelerated decoding. The following
    // parameters are also passed:
    // - |sequenceHeader|: the current OBU sequence header.
    // - |refFrames|: the pictures used as reference for decoding |pic|.
    // - |tileBuffers|: tile information.
    ...
    // and |refFrames| immediately, and |data| does not need to remain valid
    // after this method returns.
    virtual Status submitDecode(
        const AV1Picture& pic,
        const libgav1::ObuSequenceHeader& sequenceHeader,
        const AV1ReferenceFrameVector& refFrames,
  ...
  AV1Decoder(std::unique_ptr<AV1Accelerator> accelerator,
             ...
             const VideoColorSpace& containerColorSpace = VideoColorSpace());
  ~AV1Decoder() override;
  ...
  void SetStream(int32_t id,
                 webrtc::scoped_refptr<DecoderBuffer> buffer) override;
 ...
 private:
  ...
  AV1Accelerator::Status decodeAndOutputPicture(
      ...
      const libgav1::Vector<libgav1::TileBuffer>& tileBuffers);
  void updateReferenceFrames(webrtc::scoped_refptr<AV1Picture> pic);
  void clearReferenceFrames();
  // Checks that |refFrames| is consistent with libgav1's reference frame
  // state (returns false if not) and cleans old reference frames from
  // |refFrames| as needed. Also asserts that all reference frames needed by
  // |currentFrameHeader| are in |refFrames|. This method should be called
  // prior to using |refFrames| (which includes calling
  // |accelerator|->submitDecode());
  ...
  webrtc::scoped_refptr<media::DecoderBuffer> decoderBuffer;
  std::unique_ptr<libgav1::BufferPool> bufferPool;
  std::unique_ptr<libgav1::DecoderState> state;
  std::unique_ptr<libgav1::ObuParser> parser;

  const std::unique_ptr<AV1Accelerator> accelerator;
  AV1ReferenceFrameVector refFrames;

  std::optional<libgav1::ObuSequenceHeader> currentSequenceHeader;
  std::optional<libgav1::ObuFrameHeader> currentFrameHeader;
  libgav1::RefCountedBufferPtr currentFrame;

  gfx::Rect visibleRect;
  gfx::Size frameSize;
  VideoCodecProfile profile;
  VideoColorSpace containerColorSpace;
  VideoColorSpace pictureColorSpace;
  uint8_t bitDepth = 0;
  VideoChromaSampling chromaSampling = VideoChromaSampling::kUnknown;
  std::optional<gfx::HDRMetadata> hdrMetadata;

  int32_t streamId = 0;
  std::unique_ptr<DecryptConfig> decryptConfig;

  // Secure handle to pass through to the accelerator when doing secure playback
  // on ARM.
  uint64_t secureHandle = 0;

  // Pending picture for decode when accelerator returns kTryAgain.
  webrtc::scoped_refptr<AV1Picture> pendingPic;
