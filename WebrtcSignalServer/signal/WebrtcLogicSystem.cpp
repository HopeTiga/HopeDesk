    ...
    namespace signal
    {
                            ...
                            co_await func();
                                ...
                                if (ptr) {
                                    try {
                                        ...
                                        LOG_ERROR("AsyncTaskExecute Task Exception: {}", e.what());
                                    }
                                }
                                });
                            ...
                            co_return;
                                ...
                                if (ptr) {
                                    try {
                                        ...
                                        LOG_ERROR("Overload boost::asio::CoSpawn HttpTask Response Exception: {}", e.what());
                                    }
                                }
                                });
        ...
        void WebrtcLogicSystem::initHandlers() {
            ...
            std::function<boost::asio::awaitable<void>(WebrtcSignalPacket, std::string)> forwardHandler = [this](WebrtcSignalPacket webrtcSignalPacket, std::string requestTypeStr)->boost::asio::awaitable<void> {
                ...
                int64_t requestTypeValue = webrtcSignalPacket.webrtcEnvelope.requestType;

                std::string accountId(webrtcEnvelope.accountId);

                std::string targetId(webrtcEnvelope.targetId);
            ...
            webrtcHandlers[9] = std::make_unique<WebrtcHandler>([this, forwardHandler](WebrtcSignalPacket webrtcSignalPacket)->boost::asio::awaitable<void> {
                ...
                if (webrtcSignalPacket.packet.size() <= forwardEnvelopeSize) {
                    ...
                    webrtcSignalPacket.webrtcEnvelope.state = 400;

                    webrtcSignalPacket.webrtcEnvelope.message = "Forward Message Missing ForwardPacket";

                    webrtcSignalPacket.webrtcSignalSocket->asyncWrite(struct_pack::serialize<std::string>(webrtcSignalPacket.webrtcEnvelope));

                    co_return;

                }

                webrtcSignalPacket.packet.erase(0, forwardEnvelopeSize);
                    ...
                    [this](std::exception_ptr ptr) {
                        if (ptr) {
                            try {
                                ...
                                LOG_ERROR("boost::asio::CoSpawn HttpTask Response Exception: {}", e.what());
                            }
                        }
                    });
                };
